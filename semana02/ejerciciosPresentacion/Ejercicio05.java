 ...
 * - Retornar true o false indicando si el lote de transacciones es válido (es válido si NO hay transacciones no aprobadas).
 */
public class Ejercicio05 {

    // Definición de la entidad Transaction como Record para inmutabilidad y concisión
     ...
     * Un lote es válido si TODAS las transacciones están aprobadas (ninguna no aprobada).
     */
    public static boolean esLoteValido(List<Transaction> transactions) {
        // Explicación de la solución:
        // 1. .stream(): Inicia la secuencia de procesamiento.
        // 2. .peek(System.out::println): Inspecciona cada elemento en tránsito por el flujo imprimiéndolo en consola.
        // 3. .anyMatch(transaction -> !transaction.isApproved()): Evalúa si EXISTE AL MENOS UNA transacción
        //    que no esté aprobada (!isApproved). Retorna true si encuentra al menos una rechazada.
        // 4. return !hasUnapprovedTransaction: Niega el resultado anterior. Si existían no aprobadas (true),
        //    el lote no es válido (false). Si no existían no aprobadas (false), el lote es válido (true).
